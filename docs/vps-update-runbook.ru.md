...
Официальный релиз `v0.15.0` добавляет готовые образы в GitHub Container
Registry. В fork есть ежедневная Telegram-сводка, которой нет в официальных
образах. Поэтому этот VPS запускает код из archive fork командой
`docker compose up -d --build`.

Не выполнять `docker compose pull` для этого экземпляра, пока все изменения
fork не появятся в официальном релизе или не будет настроена публикация образов
собственного fork. Иначе официальные образы заменят код ежедневной сводки.
